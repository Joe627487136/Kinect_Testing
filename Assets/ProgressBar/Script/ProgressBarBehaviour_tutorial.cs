...
namespace ProgressBarBehaviour_tutorial
{
    /// <summary>
    /// This Script is directed at linearly progressing designs.
    /// </summary>
    public class ProgressBarBehaviour_tutorial : MonoBehaviour, IIncrementable, IDecrementable
    {
        /// <summary>
		...
		void Detect_and_React(){
			bool _start = Tutorial_manager.signal_bar_start_bool;
			bool _finish = Tutorial_manager.signal_bar_finish_bool;
			if (_start == false) {
				DecrementValue (1);
			}
			if (_start == true) {
        ...
        {
			print ("Bar Completed");
			progress_bar_signal = "Bar Completed";
