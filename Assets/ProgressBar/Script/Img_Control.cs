...
public class Img_Control : MonoBehaviour {
	...
	private void SavePLayerScore(){
		PlayerPrefs.SetInt (Session_ID.get_player_id(), score);
	}
	...
	void Start () {
		...
		string player_id = Session_ID.get_player_id();
		if (PlayerPrefs.HasKey (player_id)) {
			score = PlayerPrefs.GetInt (player_id);
	...
	void udt_imgs_text(){
		...
		if (playlist != null) {
			...
			if ((in_signal == "Bar 4 Completed" || in_signal == "Game Finished") && oneshot_flag[3]) {
				...
				GameObject.Find ("Musicians").SetActive (false);
				End_panel.SetActive (true);
				Result.GetComponent<Text> ().text = "Congrats! You scored: " + (final_score - starting_score).ToString ();
			}
		}
	}
